...
const submitVote = async (voteId: string, points: CandidatePoints, voteName: string, candidates: { name: string }[]): Promise<void> => {
    ...
    // Use a unique key for each user's vote to avoid overwriting
    const userVoteKey = `user-${Date.now()}`;

    // Get existing results, or an empty object if they don't exist yet
    const existingResults = await getVoteResults(voteId) || {};

    // Append the new vote results using the unique user key
    ...
    await saveVoteResults(voteId, updatedResults);
...
export default function VotePage() {
    ...
    const voteStorageKey = `${VOTE_STORAGE_KEY_PREFIX}${voteId}`;// Fetch vote details and check if user has already voted

    useEffect(() => {
        const voteId = params.voteId as string;
        console.log("Voting page voteId from useParams:", voteId); // Add this line
        if (voteId) {
            getVoteDetails(voteId)
                .then(data => {
                    if (data) {
                        ...
                        setError('Vote details not found.'); // Updated error message
                    }
                })
                .catch(err => {
